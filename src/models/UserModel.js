...
class UserModel {
									...
									connection.query(sql, { login: dataUser.login }, (err, res) => {
										if (err) {
											...
											connection.query(
												sql,
												[
													{ src_photo: '/images/upload/default-user.png', id_user: res[0].id }
												],
												(err) => {
													if (err) {
														reject(err);
							...
							connection.query(sql, [ id_user ], (err, res) => {
								if (err) {
									...
									if (!res.length) {
										...
										connection.query(
											sql,
											[ { id_user: id_user, src_photo: '/images/upload/default-user.png' } ],
											(err) => {
												if (err) {
													reject(err);
												}
	...
	static checkIfNeedingToPutPhotoFav(id_user) {
		return new Promise((resolve, reject) => {
			let sql = 'SELECT src_photo FROM users_photos_profils WHERE id_user = ?';
	...
	static modifyPhotoForNotifications(idUser, favPhoto) {
		return new Promise((resolve, reject) => {
			const sql = 'UPDATE user_notifications SET photo_user_visit = ? WHERE id_user_visit = ?';
