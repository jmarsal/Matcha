...
class SeedAccount {
    ...
    addPhotosToFolderUsers() {
        ...
        return Promise.all(this.src.map((useridSrc) => {
            count++;
            let dest = "./public/profils/" + count + "/";

            return Promise.all(useridSrc.map((x) => {
                return download(x, dest);
            }));
        }));
    }
