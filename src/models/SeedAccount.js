...
class SeedAccount {
	...
	addPopToUser() {
		return new Promise((resolve, reject) => {
			...
			while (idUser <= this.nbUsers) {
				let randomPop = Math.floor(Math.random() * 100 + 1);

				this.insertPopToDb(randomPop, idUser).catch((err) => {
					reject(err);
				});
				idUser++;
			}
	...
	insertPopToDb(randomPop, id) {
		return new Promise((resolve, reject) => {
			let sql = 'UPDATE users SET popularity = ? WHERE id = ?';

			connection.query(sql, [ randomPop, id ], (err) => {
				if (err) {
					reject(err);
				}
				resolve();
	...
	checkIfTagsUsersAndCreateIt(nbUsers) {
		return this.getRandomTagsForUsers(nbUsers).then((randomTags) => {
	...
	getRandomTagsForUsers(nbUsers) {
		return new Promise((resolve) => {
			let randomTags = [], tag = '', idUser = 1;

			while (idUser <= nbUsers) {
				let tmpTags = [], maxTags = Math.floor(Math.random() * 15 + 1);

				for (let nbTags = 0; nbTags < maxTags; nbTags++) {
	...
	makefoldersPhoto() {
		return Promise.all(
			this.basicUsers.map((user, index) => {
	...
	addPhotosToFolderUsers() {
		...
		this.pathsPhotosUsers.map((paths, index) => {
			...
			promisesDownload = [ ...promisesDownload, ...tmpPromises ];
		});
